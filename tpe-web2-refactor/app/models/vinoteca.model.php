...
class VinotecaModel extends Model {
    ...
    public function getVinos() {
        $query = $this->db->prepare('SELECT a.*, b.id_cepa, b.Nombre_cepa FROM `db_vinos` a INNER JOIN `cepa` b ON a.id_cepa = b.id_cepa');
        $query->execute();
    ...
    public function getVino($id) {
        $query= $this->db->prepare('SELECT a.*, b.id_cepa, b.Nombre_cepa,  FROM `db_vinos` a INNER JOIN `cepa` b ON a.id_cepa = b.id_cepa = c.id_bodega  WHERE `ID_vino` = ?');
        $query->execute([$id]);
